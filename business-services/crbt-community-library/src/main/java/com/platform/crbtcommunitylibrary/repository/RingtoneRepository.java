...
public interface RingtoneRepository extends JpaRepository<Ringtone, Long>, JpaSpecificationExecutor<Ringtone> {
    ...
    @Query(value = "SELECT r.* FROM ringtones r JOIN categories c ON r.category_id = c.id WHERE LOWER(c.name) = LOWER(:genre) ORDER BY RANDOM() LIMIT 1", nativeQuery = true)
    Optional<Ringtone> findRandomByGenre(@Param("genre") String genre);

    @Query(value = "SELECT * FROM ringtones ORDER BY RANDOM() LIMIT 1", nativeQuery = true)
